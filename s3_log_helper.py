...
def write_json_s3():
	log_file = open("ec2_details.json")
	...
	final_filename = f'ec2-instance-details({target_date}).json'
	...
	target_bucket = "hyper-threading-automation-logs"
	target_date = str(datetime.today().strftime('%Y-%m-%d(%H:%M:%S)'))
	final_filename = f'ebs-details({target_date}).json'

	s3_resource = boto3.resource('s3')
	folder_filename = f'ebs-details-log/{final_filename}'
	s3object = s3_resource.Object(target_bucket, folder_filename)
